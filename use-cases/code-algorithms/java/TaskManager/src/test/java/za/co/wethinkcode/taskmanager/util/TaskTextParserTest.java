...
class TaskTextParserTest {
    ...
    void parseTaskFromText_withNextWeek() {
        ...
        LocalDateTime nextWeek =
                LocalDateTime.now().withHour(0).withMinute(0).withSecond(0).withNano(0).plusDays(7);
    ...
    @Test
    void parseTaskFromText_priorityAtStartOfString_shouldBeParsed() {
        // Fix 1: priority marker at start was previously ignored
        Task task = TaskTextParser.parseTaskFromText("!urgent Fix login bug");

        assertEquals("Fix login bug", task.getTitle());
        assertEquals(TaskPriority.URGENT, task.getPriority());
    }

    @Test
    void parseTaskFromText_invalidPriorityNumber_shouldNotPolluteTile() {
        // Fix 2: !5 is invalid — should stay MEDIUM, title should not contain !5
        Task task = TaskTextParser.parseTaskFromText("Fix login bug !5");

        assertEquals(TaskPriority.MEDIUM, task.getPriority());
        assertFalse(task.getTitle().contains("!5"));
    }
}
